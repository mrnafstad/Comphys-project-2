...
double potential(double omega, double r, int coloumb )
{
	...
	if ( coloumb == 1.0 ) V = omega*r*r + (double)1.0/(double)r;
	return V;
}
...
int main( int argc, char * argv[] )
{
	...
	for ( int i = 0; i < n; i++) {
		rho[i] = rho0 + i*h;
		//Calling the potential, decide wether you want coulomb potential or not by setting the last variable to 1.0 or 0.0
		double Va = potential( 1.0,  rho[i], 1.0 );
		double Vb = potential()
		//double Vi = rho[i]*rho[i];
		for (int j = 0; j < n; j++) {
			if ( i == j ) {
				A[i][j] = 2.0 / (double) ( h*h ) + Va;
			}
			else if ( fabs( i - j ) == 1 ) {
